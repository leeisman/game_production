...
## 5. 模組詳解 (Module Details)

Monolith 雖然是單體部署，但內部邏輯清晰地劃分為不同模組。詳細的業務邏輯與狀態機設計請參閱各模組文檔：

### 5.1 遊戲核心 (GMS)
負責遊戲狀態機、週期控制與開獎邏輯。
*   [GMS 文檔](../../../module/color_game/gms/README.md)
    *   狀態機流程
    *   併發模型
    *   各階段時長配置
...
### 5.2 遊戲業務 (GS)
負責玩家下注處理、結算邏輯與錢包交互。
*   [GS 文檔](../../../module/color_game/gs/README.md)
    *   下注規則與累加機制
    *   結算流程與批次處理
    *   持久化模型

### 5.3 網關 (Gateway)
負責 WebSocket 連接管理與協議轉換。
*   [Gateway 文檔](../../../module/gateway/README.md)
    *   WebSocket 協議規範
    *   命令與錯誤碼定義
*   Gateway 直接調用 ColorGame 的 UseCase。
