...
<img src="/docs/performance/images/pprof_top_list.png" alt="pprof_top_list" width="800" />
...
<img src="/docs/performance/images/pprof_top_list_json.png" alt="pprof_top_list_json" width="800" />
...
<img src="/docs/performance/images/netstat_burst.png" alt="netstat_burst" width="800" />

**數據解讀**：

1.  **平靜期 (Idle)**:
    *   **數據**: `packets` 維持在 **1,000** 左右。
    *   **原因**: 這是 13,000 個連接的 **心跳包 (Ping/Pong)** 流量。系統處理起來非常輕鬆。

2.  **爆發期 (Burst)**:
    *   **數據**: `packets` 瞬間飆升至 **30,000** 甚至 **103,857** (10萬 PPS)。
    *   **原因**: 這是遊戲的 **廣播事件**（如開獎結果、倒數計時）。伺服器需要在極短時間內向所有在線用戶發送消息。
    *   **影響**: 這種瞬間的 PPS 爆發是對 OS 網路堆疊的最大考驗。雖然平均負載不高，但瞬間壓力可能導致 CPU `sys` 飆高或丟包。

### 6.4 結論與建議
目前的 **12,000 CCU** 已接近單機物理極限（主要是網路堆疊處理能力）。

**未來優化方向**：
若要進一步提升單機穩定性，可以考慮實作 **「分批廣播 (Batch Broadcast)」** 機制。不要在 1ms 內將 13,000 個封包全數發出，而是將其分攤在 50ms-100ms 內發送，將 10萬 PPS 的尖峰削平，降低 OS 的瞬時壓力。

若需支持更高併發（如 5萬+），建議採用 **Microservices Gateway** 架構進行水平擴展。
